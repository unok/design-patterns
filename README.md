...
このリポジトリの内容は、AIによって自動生成されたものを含んでおり、以下の点にご注意ください：

- 内容の正確性や完全性は保証できません
- 実装例やパターンの説明は参考程度としてお考えください
- 本番環境での使用は、十分な検証を行った上でご自身の責任でお願いします
- 記載内容によって生じたいかなる損害についても、責任を負いかねます
- 内容に誤りを見つけた場合は、Issue や Pull Request でご指摘いただけると幸いです

各パターンの実装方法は一例であり、実際のプロジェクトでは要件や制約に応じて適切にカスタマイズすることをお勧めします。
